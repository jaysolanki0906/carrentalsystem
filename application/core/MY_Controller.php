...
class MY_Controller extends CI_Controller
{
    ...
    * When USER has cancelled booking 
    **/
    function package_booking_template($data=array()) {
        
       
		$content='';
        
        $email_template = $this->base_model->fetch_records_from(TBL_EMAIL_TEMPLATES,array('subject'=>'package_booking','status'=>'Active'));
        ...
        if (!empty($email_template) && !empty($data)) {
			...
			//$logo_img		= '<img src="'.getcwd() . '"/uploads/site_logo/site_logo.png" class="img-responsive" style="width:30%">';
            $content     	= str_replace("__SITE_LOGO__",$logo_img,$content);
            ...
            $content     	= str_replace("__ARRIVAL_DATE__",isset($data['records'][0]->arrival_date) ? $data['records'][0]->arrival_date : '',$content);
             
             
            $content     	= str_replace("__DEPT_DATE__",isset($data['records'][0]->dept_date) ? $data['records'][0]->dept_date : '',$content);
			
            $content     	= str_replace("__ARRIVAL_TIME__",isset($data['records'][0]->arrival_date) ? $data['records'][0]->arrival_date.' & '.$data['records'][0]->arrival_time : '',$content);          
            
            $content     	= str_replace("__PAYMENT_TYPE__",isset($data['payment_mode']) ? ucfirst($data['payment_mode']) : '',$content);
            
            $content     	= str_replace("__AMOUNT__",isset($data['records'][0]->price) ? $this->config->item('site_settings')->currency_symbol.$data['records'][0]->price : '',$content);

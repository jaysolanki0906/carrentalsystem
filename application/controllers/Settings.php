	...
	function hotel_api_settings()
	{
		...
		if ($this->input->post('submit') == 'Update') {
           ...
           // $this->check_isdemo(site_url().'settings/site_settings');
			...
			// FORM VALIDATIONS
			$this->form_validation->set_rules('pagination', 'pagination', 'xss_clean|required');


			if ($this->form_validation->run() == TRUE) {

            $inputdata['pagination'] 	= $this->input->post('pagination');
			$inputdata['commission'] 	= $this->input->post('commission');

			$table_name 				= "hotel_api_settings";
			$where['id'] 				= '1';//$this->input->post('update_record_id');
                ...
                $redirect_path = "settings/hotel_api_settings";
			...
			$hotel=$this->dvbs_model->get_hotel_data();
			//echo print_r($hotel);
			//die();


			/**feching hotel api settings data**/
		//$hotel = $this->config->item('hotel');
		$this->data['record'] 	= $hotel;

				

		$this->data['pagination']				= $hotel[0]->pagination;
		$this->data['commission']				= $hotel[0]->commission;
		$this->data['gmaps'] 				= "false";
		$this->data['css_type'] 			= array('form');
		$this->data['active_class'] 		= "masterSettings";
		$this->data['title'] 				= $this->lang->line('hotel_api_settings');
		$this->data['content'] 				= "admin/settings/hotel_api_settings";

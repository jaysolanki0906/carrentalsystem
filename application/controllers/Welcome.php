...
class Welcome extends MY_Controller
{
	...
	{	
		...
		if ($this->input->post()) {
			...
			if($this->input->post('package_id') != "") {
				...
				$this->session->set_userdata('package_id', $this->input->post());
			}
            ...
            if( $this->session->userdata('package_id') != ""){
				$this->data['package_id']  = $this->session->userdata('package_id');
			}
    ...
    **/ 
     ...
     * @return page
    **/ 
    function package_passenger_summary_content() 
    {
        $page='';
        $journey_details 	= $this->session->userdata('journey_details');
     	$package_id = $this->session->userdata('package_id')['package_id'];
        $records = $this->dvbs_model->get_package_description( $package_id );
        ...
        if (!empty($records))
        {
            ...
            <ul>
            <li><strong>'.$this->lang->line('package_name').':</strong><br>'.$records[0]->package_name.'</li>
             <li><strong>'.$this->lang->line('dept_date').':</strong><br>'.$records[0]->dept_date.'</li>
             <li><strong>'.$this->lang->line('arrival_date').':</strong><br>'.$records[0]->arrival_date.'</li>';
            
            if (isset($records[0]->price)) {
            
            $label = $this->lang->line('price');
            
            $page .= '<li>
                        <strong>'.$label.':</strong>
                        <aside class="side">'. $this->config->item('site_settings')->currency_symbol.$records[0]->price.'</aside>
                     </li> </ul>';
    ...
    **/ 
	function payment()
	{
          //echo "<pre>";print_R($this->session->userdata('journey_details'));die();
            $records = $this->dvbs_model->get_home_coupons(); 
         // $coupon=$this->dvbs_model->get_home_coupons();
		if ($this->input->post()) {
		...
		if(isset($this->session->userdata('journey_details')['package_id']))
		{
			$this->data['package_id'] 			= $this->session->userdata('journey_details')['package_id'];
		}
    ...
    **/ 
	...
	function package_payment_summary_content() 
    {
        ...
        if (!empty($records) && !empty($package_id)) {
	...
	function paymentConfirmation()
	{
		if ($this->input->post()) {
            ...
            if ($inputdata['payment_type'] == "payu") {
                $this->session->set_userdata('bookinginfo', $inputdata);
                redirect(site_url().'Payumoneytest/payment'); 
            }
			...
			$table 	= "bookings";
			$inputdata['invoice_file'] = $this->data['journey_details']['booking_ref'];
			...
			if (1) {
				...
				$attachment = prepare_invoice_pdf($message,"send_email",$this->data['journey_details']['booking_ref']);
				...
				if ( sendEmail($from, $to, $sub, $message,'','',$attachment ) ) {
					...
					sendEmail( $from, $to, $sub, $message,'','',$attachment );
				}
  ...
  /**
     ...
     * @return boolean
    **/ 
	function package_paymentConfirmation()
	{
		if ($this->input->post()) {
           ...
           /*  echo "<pre>";print_r($_POST);
            ...
            $this->data['total_price']  = $total_cst;

			$inputdata['total_price'] 	= $total_cst;//$this->data['journey_details']['total_cost'];
       ...
       //COUPON
			...
			if (1) {
				...
				$message = $this->package_booking_template($this->data);
				...
				if( sendEmail($from, $to, $sub, $message,'','',$attachment) ) {
					...
					sendEmail($from, $to, $sub, $message,$attachment,'','',$attachment);
				}
                ...
                if ($this->config->item('site_settings')->sms_notification=='Yes' && $mobile_number!='') {
                    ...
                    if(!empty($sms_details))
                    {
                        ...
                        $content     	= str_replace("__TOTAL__COST__", $this->config->item('site_settings')->currency_symbol.$total_cst, $content);

...
class Dvbs_Model extends Base_Model  
{
	...
	function get_home_tourpackages() {

		$this->db->order_by("image", "random");

		$packages = $this->db->get(TBL_PREFIX.TBL_TOURPACKAGE)->result();
	...
	function get_package_description($p_id = 0) {
		
		$this->db->select('*');

       // $this->db->order_by('sort_order','asc');

        $record = $this->db->get_where(TBL_PREFIX.TBL_TOURPACKAGE,array('id' => $p_id ,'status' => 'Active'))->result();

        return $record;

       // $this->db->select('id','description');
        //categories = $this->db->get_where(TBL_TOURPACKAGE,array('id' => 0))->result();
	...
	function get_home_packages() {
		...
		$records = $this->base_model->run_query("SELECT p.*,v.image,v.name as vehicle_name,v.fuel_type,v.passengers_capacity,v.large_luggage_capacity,v.small_luggage_capacity,v.model,v.cost_starting_from FROM " . $this->db->dbprefix('package_settings') . " p, " . $this->db->dbprefix('vehicle') . " v WHERE v.id=p.vehicle_id AND p.status='Active'");
    ...
    function get_hotel_data(){
        ...
        $hotel = $this->db->query("select * from vbs_hotel_api_settings")->result();
        return $hotel;
    }
	...
	//For geting tourpackage name in dropdwon
    function get_package_name(){
       $result =  $this->db->query('SELECT id,package_name FROM vbs_tourpackage')->result_array();
        $package_name = array(); 
        foreach($result as $r) { 
            $package_name[$r['id']] = $r['package_name']; 
        } 
        $package_name[''] = 'Select package  name...'; 
        return $package_name; 
       }
	  
	  function get_home_coupons() {


    //  $data=$this->db->select('coupon_code'); 
    // $this->db->from('vbs_coupons');  
    // $where = "status='active'"; 
    // $this->db->where($where);
    // //print_r($data) ;
    //  return $this->db->get()->result();

        $query = $this->db->query("select coupon_code from vbs_coupons")->result();
       return $query;
	//$query="select coupon_code from vbs_coupons";
    //print_r($query);
    $this->db->select('coupon_id','coupon_code');
 //        $categories = $this->db->get_where(TBL_COUPONS,array('coupon_id' => 0))->result();
 //        return $categories;
	}

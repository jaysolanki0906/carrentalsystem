...
{
	function get_package_image($image='')
	{
		
		$package_img='';
		if($image=='')
		{
			$package_img=DEFAULT_PACKAGE_IMG;
		}
		else
		{
			$CI	=&	get_instance();
			
			if($image != '' && file_exists(PACKAGE_IMG_UPLOAD_PATH_URL.$image)) 
			{
				$package_img = PACKAGE_IMG_PATH.$image;
			}
			else
			{
				$package_img = DEFAULT_PACKAGE_IMG;
...
if (!function_exists('prepare_invoice_pdf')) {
    ...
    function prepare_invoice_pdf($invoice_data, $mode = "download", $invoice_name = "invoice") {
        $CI = get_instance();
        $CI->load->library('pdf');
        ...
        if ($invoice_data) {
           ...
           // $invoice_data["mode"] = $mode;
            ...
            if ($mode != "html") {
                $CI->pdf->writeHTML($invoice_data, true, false, true, false, '');
            }

            $pdf_file_name = $invoice_name.".pdf";

            if ($mode === "download") {
                $CI->pdf->Output($pdf_file_name, "D");
            } else if ($mode === "send_email") {
                $temp_download_path = getcwd() . "/uploads/invoice_files/temp/" . $pdf_file_name;
